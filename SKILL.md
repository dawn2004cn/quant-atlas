...
contact:
  email: dawn2004cn
  blog: https://blog.csdn.net/flye?type=blog
metadata:
  openclaw:
    requires:
      bins: ["python3"]
...
**作者**: dawn  
**邮箱**: dawn2004cn@163.com 
**博客**: https://blog.csdn.net/flye?type=blog  
**版本**: 1.1.3  
**许可证**: MIT

## 🆕 v1.1.3 更新 (2026-03-28)

### 核心优化
- 🎯 **中长线选股增强**：支持按市场筛选（全部、沪深、双创、北交所）
- 🔧 **选股逻辑优化**：降低评分门槛，确保每天能选出股票
- 📈 **评分系统改进**：新增基础评分、技术评分、成交量评分
- 🎨 **前端界面升级**：中长线选股页面添加市场选择功能
- 🔍 **数据源优化**：优先使用全市场缓存数据，延长缓存时间

### API 接口更新
- `/api/long-term-select`：新增 `market` 参数，支持市场筛选
- `/api/selector/run`：新增 `market` 参数，支持市场筛选

### 技术改进
- 北交所股票代码过滤条件修复（支持8、4、92开头）
- 历史数据要求从60天降低到30天
- 推荐门槛从60分降低到40分
- 支持更详细的选股评分维度
...
- 支持自定义监控股票池（`config/watchlist.json` 种子；运行时以 SQLite 为准）
- 每日自动推荐，飞书/企业微信告警
