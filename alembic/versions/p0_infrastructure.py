...
def upgrade() -> None:
    ...
    if not inspector.has_table('audit_events'):
        op.create_table(
            ...
            sa.Column('ai_evidence_json', sa.Text(), nullable=True),
            sa.Column('factor_values_json', sa.Text(), nullable=True),
            sa.Column('risk_assessment_json', sa.Text(), nullable=True),
            sa.Column('compliance_result_json', sa.Text(), nullable=True),
    ...
    if not inspector.has_table('user_role_assignments'):
        op.create_table(
    ...
    if not inspector.has_table('compliance_violations'):
        op.create_table(
    ...
    # 5. Add permissions_json column to roles table (skip if already exists)
    if not inspector.has_table('roles') or 'permissions_json' not in [c["name"] for c in sa.inspect(conn).get_columns('roles')]:
        op.add_column('roles', sa.Column('permissions_json', sa.Text(), nullable=True))
