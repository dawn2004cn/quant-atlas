...
def all_integration_cards() -> tuple[IntegrationCard, ...]:
    ...
    return (
        ...
        IntegrationCard(
            key="quantstats_tearsheet",
            layer="analytics",
            source_project="QuantStats / empyrical",
            title="绩效 Tearsheet（Omega / CVaR / Ulcer）",
            summary="无 GPL 依赖的 QuantStats 级绩效核：Omega、VaR/CVaR、尾部比、溃疡指数、修复因子；接入生产 calc_metrics 与 /api/v1/quant/tearsheet。",
            solid_note="SRP：绩效公式集中在 domain/quant/tearsheet.py，回测引擎只消费结果。",
            code_paths=(
                "app/domain/quant/tearsheet.py",
                "app/infrastructure/agent/backtest/metrics.py",
                "app/presentation/api/routes_v1_quant_capability.py",
            ),
            routes=(
        ...
        IntegrationCard(
            key="alphalens_factor_diagnostics",
            layer="analytics",
            source_project="Alphalens / Qlib IC",
            title="因子 IC / 分位收益诊断",
            summary="Spearman Rank IC、ICIR、五分位多空价差；支持单期截面与日期×标的面板。",
            solid_note="ISP：因子诊断与因子挖掘/存储分离，调用方只依赖 diagnose_factor。",
            code_paths=(
                "app/domain/quant/factor_diagnostics.py",
                "app/presentation/api/routes_v1_quant_capability.py",
            ),
            routes=(IntegrationRouteRef("量化实验室", nav_id="quant_lab"),),
        ),
        IntegrationCard(
            key="hrp_allocation",
            layer="analytics",
            source_project="PyPortfolioOpt / Lopez de Prado HRP",
            title="层次风险平价（HRP）",
            summary="单链接聚类 + 逆方差二分，补齐 MVO/风险预算之外的稳健配置方法。",
            solid_note="OCP：PortfolioOptimizer.method='hrp' 扩展，不改原有 MVO 路径。",
            code_paths=(
                "app/domain/quant/hrp.py",
                "app/domain/allocation/portfolio_optimizer.py",
            ),
