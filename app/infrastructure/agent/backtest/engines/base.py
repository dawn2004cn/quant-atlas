...
def _align(
    ...
    # Cost-aware normalization: instead of pure L1 equal-weight, preserve
    # signal intensity by scaling with total absolute exposure.
    # This prevents high-conviction days (many concurrent signals) from
    # being diluted to equal weight — a known audit finding.
    total_abs = pos.abs().sum(axis=1).clip(lower=1.0)
    scale_factor = total_abs.clip(upper=max_leverage)
    pos = pos.div(total_abs, axis=0).mul(scale_factor, axis=0)
...
def _load_optimizer(config: Dict[str, Any]) -> Optional[Callable]:
    ...
    try:
        ...
        logger.warning(
            "Failed to load optimizer '%s': %s, falling back to equal weight",
            opt_name,
            e,
        )
        return None
...
class BaseEngine(ABC):
    ...
    def run_backtest(
        ...
        validator = MarketDataQualityGate() # Initialize quality gate
        ...
        if missing_codes:
            ...
            for code, df in fetched.items():
                # Apply validation gate before caching
                validated_df = validator.validate(df, code)
                key = f"{code}_{config.get('start_date')}_{config.get('end_date')}"
        ...
        if not valid_codes:
            write_stdout_json({"error": "No valid signals generated"}, indent=None)
            sys.exit(1)
        ...
        if bench_ticker and bench_ticker != "auto" and bench_result is not None:
        ...
        write_stdout_json({k: v for k, v in m.items() if not isinstance(v, dict)})
        return m
    ...
    def _rebalance(
        ...
        if target_dir != 0 and symbol not in self.positions:
            ...
            if margin + comm > self.capital:
                ...
                if size <= 0:
                    logger.warning(
                        "Capital shrink left %s below lot size at %s (available=%.2f)",
                        symbol,
                        ts,
                        available,
                    )
