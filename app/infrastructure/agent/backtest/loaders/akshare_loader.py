...
# 回测默认后复权，避免前复权(qfq)在历史时点引入未来除权信息（前视偏差）。
# 展示层可继续用 qfq；可通过 AKSHARE_BACKTEST_ADJUST 覆盖（hfq / qfq / 空串=不复权）。
_VALID_ADJUST = frozenset({"", "qfq", "hfq", "none"})


def _backtest_adjust() -> str:
    raw = (get_runtime("AKSHARE_BACKTEST_ADJUST", "hfq") or "hfq").strip().lower()
    if raw == "none":
        return ""
    return raw if raw in _VALID_ADJUST else "hfq"
