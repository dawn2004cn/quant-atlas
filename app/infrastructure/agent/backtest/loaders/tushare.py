    ...
    def fetch(
        ...
        for code in codes:
            try:
                ...
                logger.warning("Tushare fetch failed for %s: %s", code, exc, exc_info=True)
    ...
    def _merge_basic_fields(
        ...
        for code in active_codes:
            try:
                ...
                logger.warning("Tushare daily_basic failed for %s: %s", code, exc, exc_info=True)
    ...
    def _fetch_minutes(
        ...
        for code in codes:
            try:
                ...
                if df is None or df.empty:
                    logger.warning(
                        "Empty Tushare minute data for %s (points >= 2000 required)",
                        code,
                    )
                    continue
                ...
                logger.warning(
                    "Tushare minute fetch failed for %s: %s",
                    code,
                    exc,
                    exc_info=True,
                )
