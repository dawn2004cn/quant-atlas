...
def calc_metrics(
    ...
    rf_annual = resolve_annual_risk_free_rate()
    rf_per_bar = float((1.0 + rf_annual) ** (1.0 / max(bpy, 1)) - 1.0) if rf_annual else 0.0
    excess_mean = float(port_ret.mean() - rf_per_bar)

    equity_values = equity_curve.astype(float).tolist()
    if len(equity_values) >= 2 and rf_annual == 0.0:
        from app.infrastructure.compute.native_compute import (
            calculate_max_drawdown,
            calculate_sharpe_ratio,
        )

        sharpe = float(calculate_sharpe_ratio(equity_values))
        max_dd = -float(calculate_max_drawdown(equity_values)) / 100.0
    else:
        sharpe = float(excess_mean / (vol + 1e-10) * np.sqrt(bpy))
    ...
    # Sortino — downside deviation over full sample (not only negative-return bars)
    downside = np.minimum(port_ret.to_numpy(dtype=float), 0.0)
    downside_std = float(np.sqrt((downside ** 2).sum() / max(len(port_ret), 1)))
...
def _empty_metrics(initial_cash: float) -> dict[str, Any]:
    ...
    return {
        ...
        "total_return": 0, "annual_return": 0, "max_drawdown": 0, "max_drawdown_pct": 0,
