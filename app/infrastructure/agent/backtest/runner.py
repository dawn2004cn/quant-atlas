...
def _load_module_from_file(file_path: Path, module_name: str):
    """Load a Python module from a file path via sandboxed import.

    Uses SandboxedModuleLoader from the strategy plugin system to prevent
    arbitrary code execution (CVE-mitigation for S-03).
    """
    from app.domain.strategies.plugin.loader import SandboxedModuleLoader
    return SandboxedModuleLoader().load(file_path)
...
def main(run_dir: Path) -> None:
    ...
    if not config_path.exists():
        write_stdout_json({"error": "config.json not found"}, indent=None)
        sys.exit(1)
    ...
    try:
        ...
        errors = str(exc)
        write_stdout_json({"error": f"Invalid config: {errors}"}, indent=None)
        sys.exit(1)
    ...
    if not signal_path.exists():
        write_stdout_json({"error": "code/signal_engine.py not found"}, indent=None)
        sys.exit(1)
    ...
    if engine_cls is None:
        write_stdout_json(
            {"error": "SignalEngine class not found in signal_engine.py"},
            indent=None,
        )
        sys.exit(1)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        logger.error("Usage: python -m backtest.runner <run_dir>")
        sys.exit(1)
