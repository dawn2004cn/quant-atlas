...
Responsibilities:
    ...
    2. Render a run_dir (via ``codegen.write_run_dir``) and call
       ``app.infrastructure.agent.swarm.tools.backtest_tool.run_backtest``.
    ...
    *,
    ...
    Args:
        ...
        run_backtest_fn: Injection point for tests — callable(run_dir_str)
            ...
            ``app.infrastructure.agent.swarm.tools.backtest_tool.run_backtest``. Defaults to the real
            entrypoint.
...
def _default_run_backtest_fn():
    from app.infrastructure.agent.swarm.tools.backtest_tool import run_backtest
    return run_backtest
