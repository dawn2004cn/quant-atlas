...
from sqlalchemy import String, Integer, Double, DateTime, ForeignKey, Text, SmallInteger, JSON, Date, UniqueConstraint, Index
...
class YanbaoItem(Base):
    ...
    stock_code: Mapped[Optional[str]] = mapped_column(String(16), index=True)
    org_name: Mapped[Optional[str]] = mapped_column(String(128))
    pub_date: Mapped[Optional[str]] = mapped_column(String(32))
    report_url: Mapped[Optional[str]] = mapped_column(String(1024))
...
class SignalFlagPool(Base):
    ...
    signal_strategies_sell: Mapped[Optional[str]] = mapped_column(Text)
...
class AnalysisReport(Base):
    ...
    prediction_type: Mapped[Optional[str]] = mapped_column(String(64))
    validation_status: Mapped[Optional[str]] = mapped_column(String(32), default="pending", index=True)
...
class FinGPTPrediction(Base):
    ...
    predicted_movement: Mapped[Optional[str]] = mapped_column(String(64))
    positive_factors: Mapped[Optional[str]] = mapped_column(Text)
    potential_concerns: Mapped[Optional[str]] = mapped_column(Text)
    ...
    is_correct: Mapped[Optional[int]] = mapped_column(SmallInteger)
    created_at: Mapped[Optional[datetime]] = mapped_column(DateTime, default=datetime.now, index=True)
...
class FinGPTSentiment(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    ticker: Mapped[str] = mapped_column(String(32), nullable=False, index=True)
    news_id: Mapped[Optional[str]] = mapped_column(String(128))
    ...
    key_entities: Mapped[Optional[str]] = mapped_column(String(255))
    impact_level: Mapped[Optional[str]] = mapped_column(String(32))
    summary: Mapped[Optional[str]] = mapped_column(Text)
    created_at: Mapped[Optional[datetime]] = mapped_column(DateTime, default=datetime.now, index=True)
...
class KronosModel(Base):
    ...
    hf_path: Mapped[Optional[str]] = mapped_column(String(255))
...
class KronosPrediction(Base):
    ...
    actual_json: Mapped[Optional[str]] = mapped_column(Text)
...
class OpenBBProviderConfig(Base):
    ...
    api_key_encrypted: Mapped[Optional[str]] = mapped_column(String(512))
...
class OpenBBDataCache(Base):
    ...
    timeframe: Mapped[Optional[str]] = mapped_column(String(16), index=True)
...
class QuantMLFactor(Base):
    ...
    ic_mean: Mapped[Optional[float]] = mapped_column(Double)
    icir: Mapped[Optional[float]] = mapped_column(Double)
    long_average: Mapped[Optional[float]] = mapped_column(Double)
    long_short: Mapped[Optional[float]] = mapped_column(Double)
    t_stat: Mapped[Optional[float]] = mapped_column(Double)
    metadata_json: Mapped[Optional[str]] = mapped_column(Text)
    created_at: Mapped[Optional[datetime]] = mapped_column(DateTime, default=datetime.now)
...
class AgentMarketInsight(Base):
    ...
    sentiment_score: Mapped[Optional[float]] = mapped_column(Double)
    sentiment_label: Mapped[Optional[str]] = mapped_column(String(32))
    trend_prediction: Mapped[Optional[str]] = mapped_column(String(64))
    hot_sectors: Mapped[Optional[str]] = mapped_column(Text)
...
class AgentReportInterpretation(Base):
    ...
    market_impact: Mapped[Optional[str]] = mapped_column(String(64))
    full_interpretation: Mapped[Optional[str]] = mapped_column(Text)
    metadata_json: Mapped[Optional[str]] = mapped_column(Text)
    created_at: Mapped[Optional[datetime]] = mapped_column(DateTime, default=datetime.now)
...
class NewsSymbolMeta(Base):
    ...
    industry_hint: Mapped[Optional[str]] = mapped_column(String(256))
