...
class UserLifecycleSettings(Base):
    ...
    user_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    tenant_id: Mapped[int] = mapped_column(ForeignKey("tenants.id"), nullable=False, index=True)
    notifications_json: Mapped[Optional[str]] = mapped_column(Text)
    privacy_consent_json: Mapped[Optional[str]] = mapped_column(Text)
    deletion_request_json: Mapped[Optional[str]] = mapped_column(Text)
