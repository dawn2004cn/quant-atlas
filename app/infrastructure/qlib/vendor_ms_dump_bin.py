...
# (stdlib logging + lazy ``fire`` import in __main__ only).
...
class DumpDataBase:
    ...
    def _get_source_data(self, file_path: Path) -> pd.DataFrame:
        ...
        if self.date_field_name in df.columns:
            # 清洗无效的日期数据
            df = self._clean_invalid_dates(df)
            # 使用 errors='coerce' 将无效日期转为 NaT
    ...
    def _clean_invalid_dates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if invalid_count > 0:
            logger.warning(f"Cleaned {invalid_count} invalid date rows from {df.name if hasattr(df, 'name') else 'dataframe'}")
    ...
    def _load_all_source_data(self):
        ...
        def _read_df(file_path: Path):
            ...
            if self.date_field_name in _df.columns and not np.issubdtype(
                ...
                _df[self.date_field_name] = pd.to_datetime(_df[self.date_field_name], errors='coerce')
                _df = _df.dropna(subset=[self.date_field_name])
        ...
        with ProcessPoolExecutor(max_workers=self.works) as executor:
            ...
            for _code, _df in self._all_data.groupby(self.symbol_field_name, group_keys=False):
                ...
                if _code in self._update_instruments:
                    ...
                    _end_raw = self._update_instruments[_code][self.INSTRUMENTS_END_FIELD]
                    _end_ts = pd.Timestamp(_end_raw)
                    _update_calendars = (
                        _df[_df[self.date_field_name] > _end_ts][self.date_field_name].sort_values().to_list()
                    )
