"""Pure model definitions for extended 28 strategies.

Contains all strategy classes from the original extended_28.py.
...
class EXT22_Hammer_Like(_ExtBase):
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        c = _safe(df["Close"])
        body = (c - o).abs()
        lower_shadow = (np.minimum(o, c) - low).clip(lower=0)
