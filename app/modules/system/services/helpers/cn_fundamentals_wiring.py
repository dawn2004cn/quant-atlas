...
Migration path:
  1. Replace from app.modules.system.services.helpers.cn_fundamentals_wiring import get_foo()
...
warnings.warn(
    'app.modules.system.services.helpers.cn_fundamentals_wiring is deprecated. '
...
def bind_cn_fundamentals_port(port: CnFundamentalsPort) -> None:
    global _port
    _port = port
def get_cn_fundamentals_port() -> CnFundamentalsPort:
    if _port is None:
        raise RuntimeError(
            "CnFundamentalsPort not configured; bootstrap must call bind_cn_fundamentals_port()"
        )
    return _port
