  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 lg:grid-cols-7 gap-3">
        <StatCard label="系统状态" value={snap?.overall_status ?? "—"} ok={snap?.overall_status === "healthy" || snap?.overall_status === "ok"} />
      ...
      {fullDumpCount > 0 && (
        <div className="quant-card qa-tone-banner--warn text-sm">
          ...
          {quotesApi?.backend ? ` · backend=${quotesApi.backend}` : ""}
          。请改用 <code className="font-mono text-xs">quotes/page</code>。
        </div>
      )}
      ...
      {alertOps && (
        <div className="quant-card text-sm space-y-1">
          ...
          <div className="text-xs text-[var(--quant-muted)] font-mono">
            ...
            {" · "}
            dump_monitor={alertOps.quotes_dump_monitor_beat ? `on/${alertOps.quotes_dump_monitor_beat_minutes ?? "?"}m` : "off"}
            {" · "}
        ...
        <div className="quant-card qa-tone-banner--warn text-sm">
          ...
          {snap.health_banner.quotes_full_dump_warn ? (
            <span className="ml-2 text-[10px] font-mono text-[var(--quant-muted)]">
              dump={snap.health_banner.quotes_full_dump_count ?? 0}/
              thr={snap.health_banner.quotes_full_dump_threshold ?? 1}
            </span>
          ) : null}
        </div>
      )}
      {cs && (cs.critical_missing?.length ?? 0) > 0 && (
        <div className="quant-card qa-tone-banner--danger text-sm">
          <span className="font-bold" style={{ color: "var(--tone-danger)" }}>关键服务缺失: </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="quant-card">
          ...
          <div className="flex gap-2 mb-3">
            ...
            />
            <button type="button" className="btn btn-sm btn-primary" onClick={() => void queryTrace()}>查询</button>
          </div>
          <div className="space-y-1 max-h-64 overflow-y-auto text-xs font-mono text-[var(--quant-muted)]">
            {traceResults.map((t, i) => (
              <div key={i}>{t.time} · {t.query || t.raw || t.trace_id}</div>
            ))}
            ...
            <tbody>
              {beatRuns.map((r, i) => (
                <tr key={i} className="border-b border-[var(--quant-border)]/40">
                  ...
                  <td className={`py-2 text-right ${r.ok ? "text-up" : "text-down"}`}>{r.ok ? "OK" : "FAIL"}</td>
                </tr>
              ))}
              {!beatRuns.length ? (
                <tr><td colSpan={4} className="py-6 text-center text-[var(--quant-muted)]">暂无 Beat 记录</td></tr>
              ) : null}
            </tbody>
          </table>
        </div>
