...
import { DEMO_TIER_INVESTMENT } from "../lib/demoCatalog";
...
export default function UserTiersInvestment() {
  const { data, error, isLoading, mutate } = useSWR<TierData>("/user/tiers/investment", apiFetchV1);

  const isDemo = Boolean(error) || !data || (!(data.benefits ?? []).length && !data.tier);
  const view = isDemo ? DEMO_TIER_INVESTMENT : data!;
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-[var(--quant-muted)] text-sm">Investment 会员权益</p>
