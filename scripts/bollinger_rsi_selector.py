...
class BollingerRSISelector:
    ...
    def analyze_single_stock(self, code: str) -> Dict:
        ...
        try:
            ...
            if cached_score:
                ...
                if stock_info:
                    result = {
                        ...
                        'buy_signals': [s['description'] for s in cached_score['details']['strategies']] if cached_score['details'].get('strategies') else ['布林带+RSI策略入选'],
                        ...
                        'recommend': cached_score['score'] >= 70,  # 布林带+RSI策略推荐门槛
                        'update_time': cached_score['update_time']
                    }
                    return result
            
            # 缓存中没有评分或已过期，重新计算
            logger.debug("重新计算 %s 评分 (布林带+RSI策略)", code)
            ...
            # 价格是否接近下轨
            near_bb_low = df['close'].iloc[-1] < df['BB_low'].iloc[-1] * 1.02
            # RSI是否处于超卖区
            rsi_oversold = df['RSI'].iloc[-1] < 30
            # RSI是否有向上趋势
            rsi_trend_up = df['RSI'].iloc[-1] > df['RSI'].iloc[-5]
            ...
            if near_bb_low:
                strategies.append({'description': '价格接近布林带下轨', 'score': 25})
            if rsi_oversold:
                strategies.append({'description': 'RSI处于超卖区', 'score': 25})
            if rsi_trend_up:
                strategies.append({'description': 'RSI有向上趋势', 'score': 20})
            
            details = {
                'strategies': strategies,
                'near_bb_low': near_bb_low,
                'rsi_oversold': rsi_oversold,
                'rsi_trend_up': rsi_trend_up,
            ...
            result = {
                ...
                'rating': rating,
                'details': details,
                'buy_signals': [s['description'] for s in strategies] if strategies else ['布林带+RSI策略综合评分入选'],
    ...
    def select_top_stocks(self, top_n: int = 10, market: str = 'all') -> List[Dict]:
        ...
        logger.info("重新生成当天选股报告: %s (市场: %s, 策略: 布林带+RSI)", today, market)
        ...
        def analyze_stock(code):
            # 在每个线程中创建完整的BollingerRSISelector实例，确保线程安全
            thread_selector = BollingerRSISelector()
            try:
        ...
        logger.info("返回评分最高的 %s 只股票 (布林带+RSI策略)", len(top_results))
    ...
    def generate_report(self, stocks: List[Dict]) -> str:
        report = ["="*60, f"📊 布林带+RSI策略中长线选股报告 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", "="*60, ""]
        for stock in stocks:
            report.extend([
                ...
                *["      • " + s['description'] for s in stock['details']['strategies']],
                "  📈 技术指标:",
                f"      • 价格接近布林带下轨: {stock['details'].get('near_bb_low', False)}",
                f"      • RSI处于超卖区: {stock['details'].get('rsi_oversold', False)}",
                f"      • RSI有向上趋势: {stock['details'].get('rsi_trend_up', False)}",
...
if __name__ == '__main__':
    ...
    if top_stocks:
        ...
        with open('bollinger_rsi_recommendation.txt', 'w', encoding='utf-8') as f:
            f.write(report)
    selector.close()