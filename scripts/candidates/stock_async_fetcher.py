...
class StockAsyncFetcher(DataFetcherInterface):
    """异步股票数据获取器"""
        ...
        for code in stock_codes:
            try:
                ...
                if cached:
                    ...
                    # 处理两种时间格式：带T的ISO格式和不带T的格式
                    try:
                        if 'T' in update_time:
                            cache_time = datetime.fromisoformat(update_time)
                        else:
                            cache_time = datetime.strptime(update_time, '%Y-%m-%d %H:%M:%S.%f')
                    except ValueError:
                        # 如果解析失败，尝试其他格式
                        cache_time = datetime.fromisoformat(update_time.replace(' ', 'T'))
    ...
    def get_stock_data(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            days = (end_dt - start_dt).days + 30  # 多获取30天数据以确保足够的历史数据
            
            # 获取历史数据
            df = self.ds.get_history_data(symbol, days=days)
...
def fetch_all_market():
    ...
    try:
        ...
        # 同时保存全市场数据到market_all_cache表
        print(f"💾 正在保存全市场数据到缓存...")
        # 构造全市场数据格式
        market_data = []
        for stock in stocks_data:
            ...
            volume = safe_float(stock['volume'])
            ...
            market_data.append({
                ...
                'pe': 0,  # 默认值
                'pb': 0,  # 默认值
                'total_market_cap': 0,  # 默认值
                'circulating_market_cap': 0,  # 默认值
                'limit_up_reason': '',
                'first_limit_up_time': '',
                'final_limit_up_time': ''
            })
        
        fetcher.cache.save_market_all_cache(market_data)
        
        print(f"✅ 成功保存 {len(stocks_data)} 只股票数据")
        print(f"✅ 成功保存全市场数据到缓存")
