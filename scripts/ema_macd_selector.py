...
class EMAMACDSelector:
    ...
    def analyze_single_stock(self, code: str) -> Dict:
        ...
        try:
            ...
            if cached_score:
                ...
                if stock_info:
                    result = {
                        ...
                        'buy_signals': [s['description'] for s in cached_score['details']['strategies']] if cached_score['details'].get('strategies') else ['EMA+MACD策略入选'],
                        ...
                        'recommend': cached_score['score'] >= 70,  # EMA+MACD策略推荐门槛
                        'update_time': cached_score['update_time']
                    }
                    return result
            
            # 缓存中没有评分或已过期，重新计算
            logger.debug("重新计算 %s 评分 (EMA+MACD策略)", code)
            
            # 优先从缓存中获取历史数据
            history_data = self.cache.get_stock_history(code)
            if history_data and len(history_data) >= 200:
                ...
                df = self.ds.get_history_data(code, days=300)
            ...
            # MACD是否金叉
            macd_crossover = df['MACD_line'].iloc[-1] > df['MACD_signal'].iloc[-1]
            # MACD柱状图是否由负转正
            macd_hist_positive = df['MACD_hist'].iloc[-1] > 0
            ...
            if price_above_ema200:
                strategies.append({'description': '价格在200日均线上方', 'score': 25})
            if macd_crossover:
                strategies.append({'description': 'MACD金叉', 'score': 25})
            if macd_hist_positive:
                strategies.append({'description': 'MACD柱状图由负转正', 'score': 20})
            ...
            result = {
                ...
                'rating': rating,
                'details': details,
                'buy_signals': [s['description'] for s in strategies] if strategies else ['EMA+MACD策略综合评分入选'],
    ...
    def select_top_stocks(self, top_n: int = 10, market: str = 'all') -> List[Dict]:
        ...
        logger.info("重新生成当天选股报告: %s (市场: %s, 策略: EMA+MACD)", today, market)
        ...
        def analyze_stock(code):
            # 在每个线程中创建完整的EMAMACDSelector实例，确保线程安全
            thread_selector = EMAMACDSelector()
            try:
        ...
        logger.info("返回评分最高的 %s 只股票 (EMA+MACD策略)", len(top_results))
    ...
    def generate_report(self, stocks: List[Dict]) -> str:
        report = ["="*60, f"📊 EMA+MACD策略中长线选股报告 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", "="*60, ""]
        for stock in stocks:
            report.extend([
                ...
                *["      • " + s['description'] for s in stock['details']['strategies']],
                "  📈 技术指标:",
                f"      • 价格在200日均线上方: {stock['details'].get('price_above_ema200', False)}",
                f"      • MACD金叉: {stock['details'].get('macd_crossover', False)}",
                f"      • MACD柱状图由负转正: {stock['details'].get('macd_hist_positive', False)}",
...
if __name__ == '__main__':
    ...
    if top_stocks:
        ...
        with open('ema_macd_recommendation.txt', 'w', encoding='utf-8') as f:
            f.write(report)
    selector.close()