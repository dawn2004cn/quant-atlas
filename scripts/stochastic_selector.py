...
class StochasticSelector:
    ...
    def analyze_single_stock(self, code: str) -> Dict:
        ...
        try:
            ...
            if cached_score:
                ...
                if stock_info:
                    result = {
                        ...
                        'buy_signals': [s['description'] for s in cached_score['details']['strategies']] if cached_score['details'].get('strategies') else ['随机震荡策略入选'],
                        ...
                        'recommend': cached_score['score'] >= 70,  # 随机震荡策略推荐门槛
                        'update_time': cached_score['update_time']
                    }
                    return result
            
            # 缓存中没有评分或已过期，重新计算
            logger.debug("重新计算 %s 评分 (随机震荡策略)", code)
            
            # 优先从缓存中获取历史数据
            history_data = self.cache.get_stock_history(code)
            if history_data and len(history_data) >= 50:
            ...
            # K线是否在D线上方
            k_above_d = df['K'].iloc[-1] > df['D'].iloc[-1]
            # K线是否处于超卖区
            k_oversold = df['K'].iloc[-1] < 30
            # 价格是否在SMA50上方
            ...
            if k_above_d:
                strategies.append({'description': 'K线在D线上方', 'score': 25})
            if k_oversold:
                strategies.append({'description': 'K线处于超卖区', 'score': 25})
            if price_above_sma50:
                strategies.append({'description': '价格在SMA50上方', 'score': 20})
            ...
            result = {
                ...
                'rating': rating,
                'details': details,
                'buy_signals': [s['description'] for s in strategies] if strategies else ['随机震荡策略综合评分入选'],
    ...
    def select_top_stocks(self, top_n: int = 10, market: str = 'all') -> List[Dict]:
        ...
        logger.info("重新生成当天选股报告: %s (市场: %s, 策略: 随机震荡)", today, market)
        ...
        def analyze_stock(code):
            # 在每个线程中创建完整的StochasticSelector实例，确保线程安全
            thread_selector = StochasticSelector()
            try:
        ...
        logger.info("返回评分最高的 %s 只股票 (随机震荡策略)", len(top_results))
    ...
    def generate_report(self, stocks: List[Dict]) -> str:
        report = ["="*60, f"📊 随机震荡策略中长线选股报告 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", "="*60, ""]
        for stock in stocks:
            report.extend([
                ...
                *["      • " + s['description'] for s in stock['details']['strategies']],
                "  📈 技术指标:",
                f"      • K线在D线上方: {stock['details'].get('k_above_d', False)}",
                f"      • K线处于超卖区: {stock['details'].get('k_oversold', False)}",
                f"      • 价格在SMA50上方: {stock['details'].get('price_above_sma50', False)}",
...
if __name__ == '__main__':
    ...
    if top_stocks:
        ...
        logger.info("\n%s", report)
        with open('stochastic_recommendation.txt', 'w', encoding='utf-8') as f:
            f.write(report)
    selector.close()