...
LONG_TERM_STRATEGIES = {
    ...
    'breakout_retrace': {
        ...
        'period': '10-30天',
        'params': {
            'lookback_period': 40,    # 前期高点计算周期
            'recent_period': 5,       # 近期观察周期
            'breakout_threshold': 1.02,  # 突破阈值2%
            'retrace_threshold_low': 0.98,  # 回踩下限
            'retrace_threshold_high': 1.05,  # 回踩上限
            'volume_ratio': 1.2,      # 成交量放大比例
        },
        'description': '突破前期高点后回踩确认，中短线波段操作'
    },
    'bottom_reversal': {
        'name': '底部反转',
        'type': 'long',
        ...
        'params': {
            'rsi_period': 14,
            'rsi_oversold': 50,       # RSI从超卖区回升
            'macd_fast': 12,
            'macd_slow': 26,
            'macd_signal': 9,
        },
        'description': 'RSI和MACD双重确认底部反转信号'
    },
}
...
STRATEGY_RECOMMENDATIONS = {
    ...
    'long': {
        '牛市': ['ma_trend', 'trend_following', 'breakout_retrace'],
        '熊市': ['value_growth', 'position_building', 'bottom_reversal'],
        '震荡': ['position_building', 'bottom_reversal'],  # 分批低吸
        '突破': ['macd_trend', 'trend_following', 'breakout_retrace'],
        '反弹': ['bottom_reversal', 'breakout_retrace'],
    }
}
