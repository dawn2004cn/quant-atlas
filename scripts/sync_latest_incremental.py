...
for table in ['stock_history_sh', 'stock_history_sz', 'stock_history_bj']:
    ...
    print(f"  {table}: {max_date}", flush=True)
            ...
            for row in reader:
                if len(row) >= 7:
                    try:
                        ...
                        if date_val > min_date and len(date_val) == 10 and date_val[4] == '-':
                            if has_stock_code:
                                rows.append((
                                    stock_code.lower(),
                                    date_val,
