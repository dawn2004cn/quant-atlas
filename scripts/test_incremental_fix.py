...
try:
    ...
    cn_symbol = SymbolNormalizer.normalize_cn_symbol("sh880001")
    mkt = cn_symbol[:2]
    code6 = cn_symbol[-6:]
    ...
    if p.exists():
        ...
        print(f"\n通达信最新日期: {norm_rows[-1]['date']}")
        print(f"需要同步的数据: {len(filtered)} 行")
        for r in filtered:
            print(f"  {r['date']}")
    else:
        print("通达信文件不存在")
