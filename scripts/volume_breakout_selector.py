...
class VolumeBreakoutSelector:
    ...
    def analyze_single_stock(self, code: str) -> Dict:
        ...
        try:
            ...
            if cached_score:
                ...
                if stock_info:
                    result = {
                        ...
                        'buy_signals': [s['description'] for s in cached_score['details']['strategies']] if cached_score['details'].get('strategies') else ['放量突破策略入选'],
                        ...
                        'recommend': cached_score['score'] >= 70,  # 放量突破策略推荐门槛
                        'update_time': cached_score['update_time']
                    }
                    return result
            
            # 缓存中没有评分或已过期，重新计算
            logger.debug("重新计算 %s 评分 (放量突破策略)", code)
            ...
            # 价格是否突破上轨
            price_above_bb_high = df['close'].iloc[-1] > df['BB_high'].iloc[-1]
            # OBV是否在均线上方
            obv_above_sma = df['OBV'].iloc[-1] > df['OBV_SMA'].iloc[-1]
            # 成交量是否放大
            volume_increase = df['volume'].iloc[-1] > df['volume'].rolling(window=5).mean().iloc[-1] * 1.5
            
            score = 0
            if recent_buy_signal:
                score += 30
            if price_above_bb_high:
                score += 25
            if obv_above_sma:
                score += 25
            if volume_increase:
            ...
            strategies = []
            if recent_buy_signal:
                strategies.append({'description': '最近出现买入信号', 'score': 30})
            if price_above_bb_high:
                strategies.append({'description': '价格突破布林带上轨', 'score': 25})
            if obv_above_sma:
                strategies.append({'description': 'OBV在均线上方', 'score': 25})
            if volume_increase:
                strategies.append({'description': '成交量放大', 'score': 20})
            
            details = {
                'strategies': strategies,
                'price_above_bb_high': price_above_bb_high,
            ...
            result = {
                ...
                'rating': rating,
                'details': details,
                'buy_signals': [s['description'] for s in strategies] if strategies else ['放量突破策略综合评分入选'],
    ...
    def select_top_stocks(self, top_n: int = 10, market: str = 'all') -> List[Dict]:
        ...
        logger.info("重新生成当天选股报告: %s (市场: %s, 策略: 放量突破)", today, market)
        ...
        def analyze_stock(code):
            # 在每个线程中创建完整的VolumeBreakoutSelector实例，确保线程安全
            thread_selector = VolumeBreakoutSelector()
            try:
        ...
        logger.info("返回评分最高的 %s 只股票 (放量突破策略)", len(top_results))
    ...
    def generate_report(self, stocks: List[Dict]) -> str:
        report = ["="*60, f"📊 放量突破策略中长线选股报告 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", "="*60, ""]
        for stock in stocks:
            report.extend([
                ...
                *["      • " + s['description'] for s in stock['details']['strategies']],
                "  📈 技术指标:",
                f"      • 价格突破布林带上轨: {stock['details'].get('price_above_bb_high', False)}",
                f"      • OBV在均线上方: {stock['details'].get('obv_above_sma', False)}",
                f"      • 成交量放大: {stock['details'].get('volume_increase', False)}",
                f"      • 最近出现买入信号: {stock['details'].get('recent_buy_signal', False)}",
                "💰 操作建议:",
...
if __name__ == '__main__':
    ...
    if top_stocks:
        ...
        with open('volume_breakout_recommendation.txt', 'w', encoding='utf-8') as f:
            f.write(report)
    selector.close()